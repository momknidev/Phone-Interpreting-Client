...
// GraphQL
import { TARGET_LANGUAGES } from '../../../../graphQL/queries';
import {
  CREATE_TARGET_LANGUAGE,
...
export default function SourceLanguages({ refreshKey }) {
  ...
  const [createLanguage, { loading: createLoading }] = useMutation(CREATE_TARGET_LANGUAGE);
  const [updateLanguage, { loading: editLoading }] = useMutation(UPDATE_TARGET_LANGUAGE);
  const [deleteLanguage, { loading: deleteLoading }] = useMutation(DELETE_TARGET_LANGUAGE);

  const { loading, data, error, refetch } = useQuery(TARGET_LANGUAGES, {
    variables: {
      offset: page,
      limit: rowsPerPage,
      order,
  ...
  const languages = data?.targetLanguages?.languages || [];
  ...
  return (
    <>
      <Card sx={{ p: 1 }}>
        ...
        <TablePaginationCustom
          count={data?.targetLanguages?.filteredCount || 0}
          page={page}
