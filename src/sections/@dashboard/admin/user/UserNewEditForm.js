...
export default function UserNewEditForm({ isEdit = false, currentUser }) {
  ...
  const NewUserSchema = Yup.object().shape({
    ...
    phone: Yup.string(),
    isAdmin: Yup.boolean().required('Il ruolo è obbligatorio'),
    ...
    customer: Yup.array().when('isAdmin', {
      is: true,
      then: () => Yup.array().of(Yup.string()),
      otherwise: () =>
        Yup.array()
          .of(Yup.string())
          .min(1, 'È richiesto almeno un cliente')
          .required('È richiesto almeno un cliente'),
    }),
  });
  ...
  const onSubmit = async (data) => {
    try {
        ...
        await editClient({
          variables: {
            ...
            clientDetails: {
              ...
              customer: data.isAdmin ? '' : data.customer,
      ...
      reset();
      navigate(PATH_DASHBOARD.clientUser.list);
      enqueueSnackbar(!isEdit ? 'Creazione riuscita!' : 'Aggiornamento riuscito!');
    } catch (error) {
      console.error(error);
              ...
              {/* <Controller
                ...
                render={({ field }) => (
                  <Box>
                    ...
                    <RadioGroup
                      row
                      value={String(field.value)} // Convert boolean to string explicitly
